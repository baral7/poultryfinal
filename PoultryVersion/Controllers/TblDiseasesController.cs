...
namespace PoultryVersion.Controllers
{
    ...
    public class TblDiseasesController : Controller
    {
        ...
        {
            ViewData["PoultryId"] = new SelectList(_context.TblPoultryFarms, "Id", "Name");
            return View();
        }
        ...
        public async  Task<IActionResult>FeedBack(string DiseaseID, string FeedBack)
        {
            string cleanedString = DiseaseID.Replace("\r\n", "");

            // Remove spaces (you can use regular expression)
            cleanedString = Regex.Replace(cleanedString, @"\s+", "");

            Console.WriteLine(cleanedString);
            int id = Int32.Parse(cleanedString);

            var TblDisease = await  _context.TblDiseases.FindAsync(id);
            TblDisease.FeedBack = FeedBack;
           ...
           // var TblDisease = _context.TblDiseases.Find(cleanedString);
            /* TblDisease.FeedBack= FeedBack;
             _context.TblDiseases.Update(TblDisease);*/
